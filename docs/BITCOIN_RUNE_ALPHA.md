...
RedeemLoop v0.4.2 upgrades the v0.4.0 Bitcoin Rune interface alpha into a wallet/indexer beta adapter surface with API-level settlement recheck.
...
v0.4.2 is no longer just a static adapter fixture. A merchant integration can now call real wallet transfer methods:
...
The legacy `transfer.bitcoin.psbtBase64` response still exists. In v0.4.2 it remains a fixture boundary for adapter integration tests. Real merchant flows should prefer wallet-native transfer methods first, then API-level indexer-backed proof confirmation through `POST /v1/settlement/rune/recheck/:intentId`.
...
RedeemLoop v0.4.2 把 v0.4.0 的 Bitcoin Rune interface alpha 升级为带 API-level settlement recheck 的钱包/索引器 beta adapter surface。
...
v0.4.2 不再只是静态 adapter fixture。商户集成现在可以调用真实钱包转账方法：
...
旧的 `transfer.bitcoin.psbtBase64` 响应仍然保留。在 v0.4.2 中，它仍是 adapter integration test 的 fixture boundary。真实商户流程应优先使用钱包原生转账方法，然后通过 `POST /v1/settlement/rune/recheck/:intentId` 做 API-level indexer-backed proof confirmation。

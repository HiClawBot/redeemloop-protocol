...
v0.9.0 起，POS QR 和短链创建响应会生成 `checkoutToken`、token hash 持久化记录和 hosted checkout URL。用户侧页面使用 `/v1/public/...` 端点读取 session、连接钱包、请求转账、记录 broadcast，并触发可信 EVM recheck。Public session 响应不会再次返回原始 token。
